...
func (fs *FS) GetAttr(name string, ctx *fuse.Context) (*fuse.Attr, fuse.Status) {
	...
	if status == fuse.EIO {
		fs.handler(fmt.Errorf("xrdfuse: error calling Stat: %w", err))
	}
	if status != fuse.OK {
		return nil, status
	}
...
func (fs *FS) OpenDir(name string, ctx *fuse.Context) (c []fuse.DirEntry, code fuse.Status) {
	...
	status := errorToStatus(err)
	if status == fuse.EIO {
		fs.handler(fmt.Errorf("xrdfuse: error calling Dirlist: %w", err))
	}
	if status != fuse.OK {
		return nil, status
	}
...
func (fs *FS) Open(name string, flags uint32, ctx *fuse.Context) (file nodefs.File, code fuse.Status) {
	...
	status := errorToStatus(err)
	if status == fuse.EIO {
		fs.handler(fmt.Errorf("xrdfuse: error calling Open: %w", err))
	}
	if status != fuse.OK {
		return nil, status
	}
...
func (fs *FS) Mknod(name string, mode uint32, dev uint32, ctx *fuse.Context) fuse.Status {
	...
	status = errorToStatus(err)
	if status == fuse.EIO {
		fs.handler(fmt.Errorf("xrdfuse: error calling Close: %w", err))
	}
	return status
}
...
func (fs *FS) Rename(oldName string, newName string, ctx *fuse.Context) fuse.Status {
	...
	status := errorToStatus(err)
	if status == fuse.EIO {
		fs.handler(fmt.Errorf("xrdfuse: error calling Rename: %w", err))
	}
	return status
}
...
func (fs *FS) Unlink(name string, ctx *fuse.Context) fuse.Status {
	...
	status := errorToStatus(err)
	if status == fuse.EIO {
		fs.handler(fmt.Errorf("xrdfuse: error calling RemoveFile: %w", err))
	}
	return status
}
...
func (fs *FS) Rmdir(name string, ctx *fuse.Context) fuse.Status {
	...
	status := errorToStatus(err)
	if status == fuse.EIO {
		fs.handler(fmt.Errorf("xrdfuse: error calling RemoveDir: %w", err))
	}
	return status
}
...
func (fs *FS) Mkdir(name string, mode uint32, ctx *fuse.Context) fuse.Status {
	...
	status := errorToStatus(err)
	if status == fuse.EIO {
		fs.handler(fmt.Errorf("xrdfuse: error calling Mkdir: %w", err))
	}
	return status
}
...
func (fs *FS) Chmod(name string, mode uint32, ctx *fuse.Context) fuse.Status {
	...
	status := errorToStatus(err)
	if status == fuse.EIO {
		fs.handler(fmt.Errorf("xrdfuse: error calling Chmod: %w", err))
	}
	return status
}
